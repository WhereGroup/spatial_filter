...
from .settings import SUPPORTED_STORAGE_TYPES, SUPPORTED_PROVIDER_TYPES, GROUP, FILTER_COMMENT_START, FILTER_COMMENT_STOP, \
...
def warnAboutCurveGeoms(layers: Iterable[QgsMapLayer]):
    # Curved/surface GeoPackage geometries such as MultiSurfaceZ and
    # MultiCurveZ are handled in filters.layer_geometry_sql_expression() by
    # ST_CurveToLine(...) + CastToXY(...). Older plugin versions showed a
    # warning and treated these layers as unsupported; this is no longer
    # appropriate and caused needless confusion. Keep this hook as a no-op for
    # backwards compatibility with the controller.
    return

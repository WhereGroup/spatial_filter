...
from .filters import Predicate, RemoteFilterStrategy, FilterDefinition, askApply, deleteFilterDefinition, saveFilterDefinition
...
class PredicateButton(QPushButton):
    ...
    remoteStrategyChanged = pyqtSignal(object)
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.menu.addSection(self.tr('WFS / OGC API Features'))
        self.remoteStrategyActionGroup = QActionGroup(self)
        self.remoteStrategyActionGroup.setExclusive(True)
        remote_strategy_names = {
            RemoteFilterStrategy.AUTO: self.tr('Automatisch (exakt → vereinfacht → BBOX)'),
            RemoteFilterStrategy.EXACT: self.tr('Immer exakte Geometrie verwenden'),
            RemoteFilterStrategy.SIMPLIFIED: self.tr('Vereinfachte Geometrie bevorzugen'),
            RemoteFilterStrategy.BBOX: self.tr('Immer BBOX verwenden'),
        }
        for strategy in RemoteFilterStrategy:
            action = QAction(self.menu)
            action.setCheckable(True)
            if strategy == RemoteFilterStrategy.AUTO:
                action.setChecked(True)
            action.setText(remote_strategy_names.get(strategy))
            action.remote_strategy = strategy
            action.triggered.connect(self.onRemoteStrategyChanged)
            self.remoteStrategyActionGroup.addAction(action)
        self.menu.addActions(self.remoteStrategyActionGroup.actions())
    ...
    def onRemoteStrategyChanged(self):
        self.remoteStrategyChanged.emit(self.getRemoteStrategy())

    def getRemoteStrategy(self) -> RemoteFilterStrategy:
        currentAction = self.remoteStrategyActionGroup.checkedAction()
        if currentAction:
            return currentAction.remote_strategy

    def setCurrentRemoteStrategyAction(self, strategy: int):
        for action in self.remoteStrategyActionGroup.actions():
            if action.remote_strategy == RemoteFilterStrategy(strategy):
                action.triggered.disconnect()
                action.setChecked(True)
                action.triggered.connect(self.onRemoteStrategyChanged)
...
class FilterToolbar(QToolBar):
    ...
    def setupConnections(self):
        ...
        self.predicateButton.remoteStrategyChanged.connect(self.controller.setRemoteFilterStrategy)
    ...
    def onFilterChanged(self, filterDef: Optional[FilterDefinition]):
        if filterDef:
            ...
            self.predicateButton.setCurrentRemoteStrategyAction(filterDef.remote_strategy)
            ...
            self.predicateButton.setCurrentRemoteStrategyAction(RemoteFilterStrategy.AUTO)
